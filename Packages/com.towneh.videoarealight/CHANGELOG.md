...
- New `Demo` sample (replaces `Example Scene`). One scene demonstrating the package's full feature set — area-light reflections on glossy floors, the zone mask cutting contribution at the venue boundary, and cascading probe volumes catching what the OBB can't. Generated by `Tools > VideoAreaLight > Build Demo Scene` after import; the builder also auto-bakes both probe volumes so the scene is play-ready.
